...
class SystemFunctionLayer2Test {
    ...
    void layer2_cosSinCscMocked(TestPoint tp) {
        ...
        UnaryFunction cscMock = mock(UnaryFunction.class);
        ...
        when(cscMock.apply(anyDouble())).thenAnswer(inv -> TestPoint.forX(inv.getArgument(0)).csc);
        ...
        SystemFunction system = new SystemFunction(
                sinMock, cosMock, tan, cot, sec, cscMock,
                ln, log2, log3, log10);
        ...
        if (tp.x <= 0) {
            ...
            verify(cscMock, atLeastOnce()).apply(anyDouble());
        } else {
            verify(cosMock, never()).apply(anyDouble());
            verify(sinMock, never()).apply(anyDouble());
            verify(cscMock, never()).apply(anyDouble());
        }
    }
